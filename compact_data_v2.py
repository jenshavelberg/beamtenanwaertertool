...
for bl, entries in d.items():
    ...
    for e in entries:
        ...
        bl_data.append([
            ...
            round(e['soli_verh'], 2),
            round(e['kist_verh'], 2),
            round(e['kvSatz_ledig'], 2),
            round(e['kvSatz_verh'], 2),
        ])
    compact[bl] = bl_data

result = json.dumps(compact, ensure_ascii=False, separators=(',', ':'))
print(f'Compact size: {len(result)} chars, Entries: {sum(len(v) for v in compact.values())}')

with open('/Users/jens/Documents/git/beamtenanwaertertool/besoldung_compact_v2.json', 'w') as f:
    f.write(result)
